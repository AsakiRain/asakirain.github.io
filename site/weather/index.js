...
function init() {
  let foreHoursHtml = "";
  for (let i = 0; i < 24; i++) {
    foreHoursHtml += `<div class="foreHoursEach ${
      i % 2 == 0 ? "foreHoursEachDark" : "foreHoursEachLight"
    }">
  ...
  let foreDaysHtml = "";
  for (let i = 0; i < 7; i++) {
    foreDaysHtml += `<div class="foreDaysEach ${
      i % 2 == 0 ? "foreDaysEachDark" : "foreDaysEachLight"
    }">
  ...
  airChart = echarts.init(document.getElementById("airChart"), null, {
    renderer: "svg",
  });
  ...
  foreDaysChart = echarts.init(document.getElementById("foreDaysChart"), null, {
    renderer: "svg",
  });
  ...
  document.getElementById("updateTime").onclick = function () {
    //onclick不是一个函数，是方法，这里运行即使函数
    refresh();
  };
...
function refresh() {
  ...
  Promise.all([
    ...
    getData("https://devapi.qweather.com/v7/weather/10d", LOCATION),
...
function parseData(results) {
  ...
  let data_10d = results[2];
  let data_air = results[3];

  let uvIndex = data_10d["daily"][0]["uvIndex"];
  let cardData = {
    ...
    summary: getSummary({
      dnWeather: [
        data_10d["daily"][0]["textDay"],
        data_10d["daily"][0]["textNight"],
      ],
      dnTemp: [
        data_10d["daily"][0]["tempMax"],
        data_10d["daily"][0]["tempMin"],
      ],
    }),
    nowUV:
      uvIndex >= 10
  ...
  for (let i = 0; i < Object.keys(data_24h["hourly"]).length; i++) {
    foreHoursHtml += `<div class="foreHoursEach ${
      i % 2 == 0 ? "foreHoursEachDark" : "foreHoursEachLight"
                                ...
                                <div class="foreHoursTime">${data_24h["hourly"][
                                  i
                                ]["fxTime"].substr(11, 5)}</div>
                                <div class="qicon foreHoursQicon qi-${
                                  data_24h["hourly"][i]["icon"]
                                }"></div>
                                <div class="foreHoursTemp">${
                                  data_24h["hourly"][i]["text"]
                                }</div>
                            </div>`;
  }
  let foreDaysHtml = "";
  ...
  let maxTemps = [];
  let minTemps = [];
  for (let i = 0; i < Object.keys(data_10d["daily"]).length; i++) {
    foreDaysHtml += `<div class="foreDaysEach ${
      i % 2 == 0 ? "foreDaysEachDark" : "foreDaysEachLight"
    }">                   
                                <div class="foreDaysWeek">${
                                  i == 0
                                    ? "今天"
                                    : getWeek(data_10d["daily"][i]["fxDate"])
                                }</div>
                                <div class="foreDaysDate">${data_10d["daily"][
                                  i
                                ]["fxDate"].substr(5, 5)}</div>
                                <div class="qicon foreDaysQicon qi-${
                                  data_10d["daily"][i]["iconDay"]
                                }"></div>
                                <div class="foreDaysWeather">${
                                  data_10d["daily"][i]["textDay"]
                                }</div>
                            </div>`;
  }
  for (let i = 0; i < Object.keys(data_10d["daily"]).length; i++) {
    dates[i] =
      (i == 0 ? "今天" : getWeek(data_10d["daily"][i]["fxDate"])) +
      "\n" +
      data_10d["daily"][i]["fxDate"].substr(5, 5);
  }
  for (let i = 0; i < Object.keys(data_10d["daily"]).length; i++) {
    maxTemps[i] = data_10d["daily"][i]["tempMax"];
    minTemps[i] = data_10d["daily"][i]["tempMin"];
  }
  let foreDaysData = {
    foreDaysHtml: foreDaysHtml,
    dates: dates,
  ...
  let todayData = {
    ...
    precip: data_10d["daily"][0]["precip"] + "mm",
    humidity: data_10d["daily"][0]["humidity"] + "%",
    pressure: data_10d["daily"][0]["pressure"] + "hPa",
    vis: data_10d["daily"][0]["vis"] + "KM",
  };
  updateCard(cardData);
  updateForeHours(foreHoursHtml);
  updateForeDays(foreDaysData);
...
function getSummary(data) {
  ...
  summary +=
    "最高温" + data["dnTemp"][0] + "℃，最低温" + data["dnTemp"][1] + "℃。";
  return summary;
}
...
function updateForeDays(foreDaysData) {
  document.getElementById("foreDaysScroll").innerHTML =
    foreDaysData["foreDaysHtml"];
